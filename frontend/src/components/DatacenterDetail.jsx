    ...
    <div>
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            {dc.active && monthlyCostUsd > 0 && (
              <div className="text-right">
                ...
                <div className="text-[15px] font-bold text-white">${monthlyCostUsd.toFixed(2)}</div>
              </div>
            )}
            <motion.button
      ...
      {!dc.active && (
        <div className="mt-5 p-4 rounded-xl bg-gray-50 border border-dashed border-gray-300 text-center">
          <p className="text-[12px] text-gray-500">Planned datacenter — racks provisioned when needed.</p>
        </div>
      )}
    </div>
  )
