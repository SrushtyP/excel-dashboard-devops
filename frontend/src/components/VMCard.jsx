...
function Row({ label, value, accent }) {
  return (
    <div className="flex items-center justify-between py-1 border-b border-gray-50 last:border-0 gap-2 min-w-0">
      <span className="text-[9px] text-gray-400 uppercase font-bold tracking-wide flex-shrink-0">{label}</span>
      <span className="text-[10px] font-semibold text-right truncate" style={{color:accent||'#374151',maxWidth:100}}>{value}</span>
    </div>
  )
}
...
function Meter({ label, pct, color, delay }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[9px] text-gray-400 w-6 text-right flex-shrink-0">{pct}%</span>
    </div>
  )
}
...
export default function VMCard({ vm, index, onRequestStateChange }) {
  ...
  // Prefer USD fields, fall back to INR/84
  ...
  return (
    <motion.div layout
      initial={{opacity:0,x:-10,scale:0.97}} animate={{opacity:1,x:0,scale:1}}
      ...
      style={{background:cardBg,
        boxShadow:open?`0 4px 16px ${glowColor}22,0 1px 3px rgba(0,0,0,0.05)`:`0 1px 3px rgba(0,0,0,0.04)`,
      ...
      <motion.button onClick={()=>!dummy&&setOpen(o=>!o)} disabled={dummy}
        ...
        {!dummy && (
          <div className="flex-shrink-0 text-right ml-1 min-w-[52px]">
            ...
            <div className="text-[12px] font-black leading-tight" style={{color:'#1A4780'}}>{usd(optUsd)}</div>
            {savUsd>0 && <div className="text-[8px] font-bold leading-tight" style={{color:'#1EA03C'}}>-{usd(savUsd)}</div>}
          </div>
        )}
              ...
              <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.06}} className="mb-3">
                <div className="grid grid-cols-2 gap-x-3">
                  <div>
                    ...
                    <Row label="CPU"  value={`${vm.cpu} vCPU`} />
                    <Row label="RAM"  value={`${vm.memGb} GB`} />
                    <Row label="Disk" value={`${vm.diskGb} GB`} />
                    <Row label="Env"  value={vm.environment} />
                  </div>
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">Cost (USD)</p>
                    <Row label="Hourly"  value={`$${(+hrUsd).toFixed(4)}`}  accent="#1A4780" />
                    <Row label="Full/mo" value={usd(fullUsd)} accent="#1A4780" />
                    <Row label="Opt/mo"  value={usd(optUsd)}  accent="#166534" />
                    <Row label="Saves"   value={usd(savUsd)}  accent="#1EA03C" />
                  </div>
                </div>
              ...
              {(vm.state==='running'||vm.state==='snoozed') && (
                <motion.div initial={{opacity:0,y:3}} animate={{opacity:1,y:0}} transition={{delay:0.1}} className="mb-3">
                  ...
                  <div className="space-y-1.5">
                    <Meter label="CPU"  pct={cpuPct}  color="#1A4780" delay={0.12} />
                    <Meter label="RAM"  pct={memPct}  color="#00ABA9" delay={0.15} />
                    <Meter label="Disk" pct={diskPct} color="#1EA03C" delay={0.18} />
                  </div>
                </motion.div>
              )}
